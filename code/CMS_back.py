...
class ClusterManager:
    def __init__(self, G, O):
        ...
        node_to_cluster = {cluster.c_id: cluster for cluster in self.clusters}
        self.edges = []
        for u,v in G.edges():
            edge = (min(node_to_cluster[u], node_to_cluster[v], key=lambda x: x.c_id), max(node_to_cluster[u], node_to_cluster[v], key=lambda x: x.c_id))
            if edge not in self.edges:
                self.edges.append(edge)
    ...
    def add_cluster(self, nodes):
        ...
        for neighbor in list(neighbors):
            for c in self.clusters:
                if neighbor in c.nodes:
                    self.edges.append((cluster, c))
        self.next_id += 1
        return cluster
