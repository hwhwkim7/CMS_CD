...
class Cluster:
    def __init__(self, cluster_id, nodes, G, O):
...
class ClusterManager:
    ...
    def calculate_cms(self, c, alpha, m, connect=[]):
        ...
        if len(connect) == 0:
            ...
            l = len(connect + c[0].edges + c[1].edges)
...
def run(G, alpha, com):
    ...
    while len(C.clusters) > com:
        ...
        if len(S) == 0: continue
        ccms, torc, edge, c_p, c_q = max(S, key=lambda x: x[0])
        C.merge_clusters(c_p, c_q, torc, edge)

    end_time = time.time()
    communities = [c.nodes for c in C.clusters]

    return communities, orc_time, end_time-start_time
